...
typedef struct BeaconData {
  ...
  uint8_t state = NOTHING; // Initial state
} BeaconData;
...
//  Temps_start_us = millis();
  ...
  for (uint8_t i = 0; i < nb_detected; i++) {  
    if (buffer[i].moveFlag == 1 && buffer[i].timeSinceLastClick <= 25) {
      buffer[i].state = MOVE_AND_BTN;
    }
    else if (buffer[i].moveFlag == 1) {
      buffer[i].state = MOVE;
    }
    else if ( buffer[i].timeSinceLastClick <= 25) {
      buffer[i].state = BTN;
    }
    else{
  ...
  // Stock data of beacon to send
  for (uint8_t i = 0; i < nb_detected; i++) {
    if (buffer[i].state != NOTHING && strlen(buffer[i].address) != 18) // Check if it is a state to send  and a valid mac adress
    {
      ...
      tabToSend[beaconArray].moveFlag = buffer[i].moveFlag;
      beaconArray ++;
    }
  }
...
//  Serial.println(Duree_us);
  ...
  for (uint8_t w = 0; w < beaconArray; w++)
  {
    ...
    Serial.println(tabToSend[w].moveFlag);
